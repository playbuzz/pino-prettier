...
module.exports = ({
    process,
    path = require('path'),
    configFile = path.join(__dirname, 'pino-pretty.config.js'),
    ...
    prettyUtil.prettifyMessage = args => {
        ...
        return caller
            //TBD: support non-colorized mode?
            ? `\u001B[38;5;${selectColor(caller)};1m${caller}\u001B[0m : ${prettifyMessage(args)}`
            : `\u001b[1m${name}\u001b[22m : ${prettifyMessage(args)}`;
    };
